...
def generate_label_prompt(utterances):
    ...
    prompt = f"""Please generate a user intent label (action(verb)_nouns) based on all utterances in {utterances}.
The last noun should be the most central word from the utterances.
Answer the result by exactly one word.
For example:
For utterance:What is the H-index of persons who have authored a publication from 2024 onward?
The answer is: show_author_H-index
"""
    return prompt
...
def process_dataset(input_file, output_file):
    ...
    total_groups = len(df['label'].unique())
    processed_count = 0
    
    # Process each label group
    for label, group in grouped:
        processed_count += 1
        print(f"\nProcessing label group {processed_count}/{total_groups}: {label}")
        ...
        if generated_label:
            ...
            df.loc[df['label'] == label, 'gen_label'] = generated_label
            # Save the updated results immediately
            df.to_csv(output_file, sep='\t', index=False)
            print(f"Updated results saved to {output_file}")
        else:
            print(f"Warning: No label generated for group {label}")
    
    print(f"\nProcessing completed. Final results saved to {output_file}")
    print(f"Processed {processed_count} label groups")

if __name__ == "__main__":
    input_file = "train_balanced.tsv"
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"outputs/train_balanced_with_generated_labels_{timestamp}.tsv"
